...
export const MultimediaPost = ({ username }: { username: string }) => {
  ...
  const { loadMoreMultimediaPosts, multimediaPosts, multimediaPostsLoading } =
    useMultimediaPosts(username);
  ...
  }, [loadMoreMultimediaPosts, multimediaPosts?.nextCursor]);
  return (
    <>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-2 ">
        {multimediaPosts?.data.map((post, idx) => {
          return (
            <div
              key={idx}
        ...
        >
          ...
          <DialogContent className="border-primary-dark/20 p-10">
            <div className=" ">{postId && <PostDetail postId={postId} />}</div>
          </DialogContent>
        </Dialog>
      </div>
      <div ref={loaderRef} className="flex justify-center py-8">
        {(!multimediaPosts ||
          (multimediaPosts?.hasMore && multimediaPostsLoading)) && (
        ...
        {!multimediaPostsLoading &&
          (!multimediaPosts ||
            (!multimediaPosts?.hasMore &&
              multimediaPosts?.data.length > 0)) && (
